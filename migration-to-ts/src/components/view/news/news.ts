...
class News {
    ...
    private drawNewsClone(newsClone: DocumentFragment, item: Article): void {
        const newsMetaPhoto = this.getElement(newsClone, '.news__meta-photo');
        const newsMetaAuthor = this.getElement(newsClone, '.news__meta-author');
        const newsMetaDate = this.getElement(newsClone, '.news__meta-date');
        const newsDescTitle = this.getElement(newsClone, '.news__description-title');
        const newsDescSource = this.getElement(newsClone, '.news__description-source');
        const newsDescContent = this.getElement(newsClone, '.news__description-content');
        const newsReadMoreLink = this.getElement(newsClone, '.news__read-more a');
